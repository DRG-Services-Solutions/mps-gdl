        ...
        <div class="flex items-center justify-between">
            <div>
                <h2 class="font-semibold text-2xl text-gray-800 leading-tight">
                    <i class="fas fa-edit mr-2 text-indigo-600"></i> Editar Instrumental
                </h2>
                <p class="text-sm text-gray-600 mt-1">Modificando: <span class="font-mono font-medium">{{ $instrument->serial_number }}</span></p>
            </div>
                    ...
                    <div class="p-6 space-y-6">
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <input type="text" name="serial_number" id="serial_number" value="{{ old('serial_number', $instrument->serial_number) }}" required
                                ...
                                <div>
                                    ...
                                    <input type="text" name="code" id="code" value="{{ old('code', $instrument->code) }}"
                                ...
                                <div>
                                    ...
                                    <select name="category_id" id="category_id" required
                                        ...
                                        @foreach($categories as $cat)
                                            <option value="{{ $cat->id }}" {{ old('category_id', $instrument->category_id) == $cat->id ? 'selected' : '' }}>
                                                {{ $cat->name }}
                                            </option>
                                        @endforeach
                                    </select>
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <select name="product_id" id="product_id" placeholder="Buscar producto...">
                                        @if($instrument->product_id && $instrument->product)
                                            <option value="{{ $instrument->product_id }}" selected>
                                                {{ $instrument->product->code ?? '' }} - {{ $instrument->product->name }}
                                            </option>
                                        @endif
                                    </select>
                                ...
                                <div>
                                    ...
                                    <select name="depends_on_id" id="depends_on_id" placeholder="Buscar instrumento...">
                                        @if($instrument->depends_on_id && $instrument->dependsOn)
                                            <option value="{{ $instrument->depends_on_id }}" selected>
                        ...
                        <div>
                            ...
                            <textarea name="notes" id="notes" rows="3" placeholder="Observaciones adicionales..."
                                      class="w-full rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-indigo-500">{{ old('notes', $instrument->notes) }}</textarea>
                            @error('notes')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
