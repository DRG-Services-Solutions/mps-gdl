<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-semibold leading-tight text-gray-800">
                Editar Prearmado: {{ $surgicalKit->name }}
            </h2>
    ...
    <div class="py-6">
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            ...
            <div x-data="surgicalKitForm({{ $products->toJson() }}, {{ $surgicalKit->items->toJson() }})" 
                 x-init="initProducts()" 
                 class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                <form method="POST" action="{{ route('surgical-kits.update', $surgicalKit) }}">
                    @csrf
                    @method('PUT')
                            ...
                            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                                ...
                                <div>
                                    ...
                                    <input type="text" 
                                           name="name" 
                                           id="name" 
                                           value="{{ old('name', $surgicalKit->name) }}"
                                           required
                                ...
                                <div>
                                    ...
                                    <input type="text" 
                                           name="surgery_type" 
                                           id="surgery_type" 
                                           value="{{ old('surgery_type', $surgicalKit->surgery_type) }}"
                                           required
                                ...
                                <div class="md:col-span-2">
                                    ...
                                    <textarea name="description" 
                                              id="description" 
                                              rows="3"
                                              class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">{{ old('description', $surgicalKit->description) }}</textarea>
                                    @error('description')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <div>
                                    <label class="flex items-center">
                                        <input type="checkbox" 
                                               name="is_active" 
                                               value="1"
                                               {{ old('is_active', $surgicalKit->is_active) ? 'checked' : '' }}
    ...
    @push('scripts')
    <script>
        function surgicalKitForm(availableProducts, existingItems) {
            return {
                ...
                initProducts() {
                    this.selectedProducts = this.existingItems.map(item => {
                        const product = this.availableProducts.find(p => p.id === item.product_id);
                        return {
                            product_id: item.product_id,
                            code: product ? product.code : item.product.code,
                            name: product ? product.name : item.product.name,
