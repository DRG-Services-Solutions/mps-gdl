        ...
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-bold text-gray-900 leading-tight">
                    {{ __('Administración de Especialidades Médicas') }}
                </h2>
                <p class="mt-1 text-sm text-gray-600">
                    {{ __('Clasificación de productos por área de aplicación clínica (ej: Artroscopia, Osteosíntesis).') }}
                </p>
            </div>
        </div>
    </x-slot>
                ...
                <div class="px-6 py-4 bg-gradient-to-r from-teal-50 to-cyan-50 border-b border-gray-200">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center space-x-3">
                            <i class="fas fa-stethoscope w-8 h-8 text-teal-600 text-2xl"></i> 
                            <div>
                                <h3 class="text-lg font-semibold text-gray-900">{{ __('Lista de Especialidades') }}</h3>
                                {{-- Asegúrate de que $specialties esté paginado para usar total() --}}
                                <p class="text-sm text-gray-600">{{ $specialties->total() }} {{ __('especialidades registradas') }}</p>
                            </div>
                        </div>
                            ...
                            @foreach ($specialties as $specialty)
                                <tr class="hover:bg-gray-50 transition-colors duration-200">
                                    ...
                                    <td class="px-6 py-4 text-sm text-gray-700 max-w-xl">
                                        {{ Str::limit($specialty->description ?? 'Sin descripción.', 70) }}
                                    </td>
                                    <td class="px-6 py-4 text-center text-sm text-gray-700">
                                        {{-- products_count viene del withCount en el controlador --}}
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-teal-100 text-teal-800">
                                            {{ $specialty->products_count }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 text-right">
                                        <div class="flex items-center justify-end space-x-2">
                                            {{-- Botón Editar --}}
                                            <a href="{{ route('specialties.edit', $specialty->id) }}" class="px-3 py-2 text-sm text-gray-700 bg-white border rounded hover:bg-gray-50">
                                                <i class="fas fa-edit mr-1"></i> {{ __('Editar') }}
                                            </a>
                                            {{-- Formulario para Eliminar --}}
                                            <form action="{{ route('specialties.destroy', $specialty->id) }}" method="POST"
                                                  onsubmit="return confirm('¿Estás seguro de que deseas eliminar esta especialidad? Los productos asociados se desvincularán.');">
                                                @csrf
                                                @method('DELETE')
