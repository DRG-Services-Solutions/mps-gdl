<x-app-layout>
    <x-slot name="header">
        <h2 class="text-2xl font-bold text-gray-900 leading-tight">
            {{ __('Editar Especialidad:') }} <span class="text-indigo-600">{{ $specialty->name }}</span>
        </h2>
    </x-slot>
                    ...
                    <form action="{{ route('specialties.update', $specialty->id) }}" method="POST">
                        ...
                        <div class="mb-5">
                            ...
                            <textarea name="description" id="description" rows="3"
                                      ...
                                      placeholder="Detalles sobre las áreas o procedimientos que abarca esta especialidad.">{{ old('description', $specialty->description) }}</textarea>
                            @error('description')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
